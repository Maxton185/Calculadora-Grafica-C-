...
// Componentes para cambio de temas
#include <QMenu>       
#include <QAction>     
...
// ============================================================================
// SUBCLASE PERSONALIZADA DE QCHARTVIEW 
// ============================================================================
...
public:
    WindowGraficadora(QWidget *parent = nullptr) : QWidget(parent) {
        ...
        connect(btnLimpiar, &QPushButton::clicked, [this]() {
            btnLimpiar->setEnabled(false);//nuevo
            listaFunciones->clear();
            chart->removeAllSeries();
            axisX->setRange(-10, 10);
            axisY->setRange(-10, 10);
            btnLimpiar->setEnabled(true); //nuevo
        });
        ...
        connect(btnGraficar, &QPushButton::clicked, [this]() {
            btnGraficar->setEnabled(false); //nuevo
            this->procesarYRenderizar();
            btnGraficar->setEnabled(true);//nuevo
        });
...
private:
    void procesarYRenderizar() {
        ...
        for (int k = 0; k < totalFunciones; ++k) {
            ...
            QList<QPointF> puntosSegmento; // nuevo
            ...
            for (x_variable = minX; x_variable <= maxX + (delta / 2.0); x_variable += delta) {
                ...
                if (!std::isfinite(y_calculado) || std::abs(y_calculado) > LIMITE_Y_SEGURO) {
                    if (enSegmentoValido && serieActual && !puntosSegmento.isEmpty()) {//nuevo
                        serieActual->replace(puntosSegmento);//nuevo
                        puntosSegmento.clear();//nuevo
                    } //nuevo
                ...
                puntosSegmento.append(QPointF(x_variable, y_calculado));//nuevo(borre el if anterior)
    ...
    void calcularYMarcarRaices() {
        ...
        for (int k = 0; k < listaFunciones->count(); ++k) {
            ...
            for (double x_val = minX; x_val <= maxX; x_val += delta) {
                ...
                QCoreApplication::processEvents(QEventLoop::AllEvents, 2);//nuevo
                x_variable = x_val;
